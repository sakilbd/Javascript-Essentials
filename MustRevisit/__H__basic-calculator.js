...
// this one works as .eval used but should not be used 
...
// this one has some progress wihout .eval  but last test case failed 
const calculate = (s) => {
    // return eval(s)
    if (s == 0) {
        return 0;
    ...
    for (let i = 0; i < length; i++) {
        if (s[i] == "0") {
            num += 0;
            i == length - 1 ? numStack.push(parseInt(num.split("").join(""))) : "";
        } else {
            if (parseInt(s[i])) {
                num += s[i];
                i == length - 1 ? numStack.push(parseInt(num.split("").join(""))) : "";
